...
namespace LINQTest
{
    ...
    internal class Select
    {
        ...
        public void SelectManyGoodExample()
        {
            ...
            var rez =  list.Select(w => w.Country).ToList();

            var listOfArrays = list.Select(w => w.Years).ToList();
            var listOfArraysSelectMany = list.SelectMany(w => w.Years, (w,y) => new { Country = w.Country, Year = y }).ToList();

            foreach (var arr in listOfArraysSelectMany)
            {
